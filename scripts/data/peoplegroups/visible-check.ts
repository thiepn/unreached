...
import { buildVisibleCountryRecords, entityGsecRange, entityResourceBreakdown, entityTaxonomy } from "../../../src/providers/peoplegroups/visible.js";
...
if (first.reach.classification !== "mixed") throw new Error("PEID with GSEC 2 and 5 must retain mixed source status.");
if (first.population.knownValue !== 120000 || first.population.complete) throw new Error("Visible PEID population must remain an explicitly partial known-context sum.");
const range = entityGsecRange(first);
if (!range || range.min !== 2 || range.max !== 5) throw new Error("Visible GSEC range did not preserve country-context values.");
const taxonomy = entityTaxonomy(first);
if (taxonomy.peopleCluster !== "Example Cluster" || taxonomy.affinityBloc !== "Example Bloc") throw new Error("Visible source taxonomy resolution failed.");
...
if (filterLivePeople(entities, { ...baseFilter, countryIso3: "NGA" }).map((entity) => entity.peid).join() !== "7001") throw new Error("Live country filter failed.");
if (filterLivePeople(entities, { ...baseFilter, query: "PG007003" }).map((entity) => entity.peid).join() !== "7002") throw new Error("Live PGID search failed.");
if (filterLivePeople(entities, { ...baseFilter, status: "unreached-only" }).map((entity) => entity.peid).join() !== "7002") throw new Error("Live GSEC rollup filter failed.");
...
if (!isLivePrayerEligible(first)) throw new Error("A PEID with at least one GSEC 0–3 context must be prayer-eligible.");
...
console.log(`U12C visible real-data checks passed: ${entities.length} PEIDs, ${countries.length} countries, source-native GSEC/resources, certified prayer templates.`);
