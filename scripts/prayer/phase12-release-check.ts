...
for (const marker of ["PRAYER_LIBRARY_BATCH_SIZE = 24", "Showing {visible.length} of {scoped.length}", "Show {Math.min(PRAYER_LIBRARY_BATCH_SIZE, remaining)} more"]) {
  if (!prayPage.includes(marker)) throw new Error(`Phase 12 library progressive-disclosure contract missing: ${marker}`);
}
if (prayPage.includes("slice(0, 60)") || prayPage.includes("Showing up to 60")) throw new Error("Phase 12 must not silently cap the library at 60 records.");

for (const marker of ['id: "short", label: "Short", prompts: 3', 'id: "standard", label: "Standard", prompts: 5', 'id: "extended", label: "Extended", prompts: 7', "No timer runs"]) {
  if (!focusPage.includes(marker)) throw new Error(`Phase 12 guide-length contract missing: ${marker}`);
}
