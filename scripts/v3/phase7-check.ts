...
if (regionModel.includes("RegnSub") || regionModel.includes("Regn:")) {
  throw new Error("V3 Phase 7 must not use PeopleGroups region text as canonical geographic identity.");
}
...
for (const marker of ['| "regions"', '"/regions": "regions"', 'path.match(/^\\/regions\\/([a-z0-9-]+)$/i)', "regionSlug"]) {
  requireText(router, marker, "region route");
}
...
for (const marker of [
  ...
  "region route continues into countries",
  "country breadcrumb is World Region Country",
  "country continues into people",
  "geographic hierarchy stays usable on mobile",
]) requireText(browser, marker, "Phase 7 browser certification");
