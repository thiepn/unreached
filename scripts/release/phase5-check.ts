...
if (packageJson.version !== "2.1.1") throw new Error("Phase 5: final release must remain 2.1.1.");
...
const releaseNotes = await read("docs/releases/v2.1.1.md");
for (const marker of ["Unreached v2.1.1", "Release publication is exact-SHA gated", "physical-hardware testing", "branch protection"]) {
  requireText(releaseNotes, marker, `v2.1.1 release-note topic ${marker}`);
}
const releaseWorkflow = await read(".github/workflows/publish-v2.1.1.yml");
for (const marker of ["workflow_run:", "Deploy Unreached to GitHub Pages", "v2.1.1", "unreached/pages-production", "unreached/peoplegroups-live", "unreached/worker-production", "POST", "/releases"]) {
  requireText(releaseWorkflow, marker, `exact-SHA release workflow marker ${marker}`);
}
...
console.log("Phase 5 final-release checks passed: 12 reviewed profiles retain cited/nuanced current-safe content, prayer template review is current, PWA packaging is complete, device-class acceptance is wired, and exact-SHA release/maintenance governance is present.");
