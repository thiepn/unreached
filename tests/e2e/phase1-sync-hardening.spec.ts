...
async function seedBoundDevice(page: Page, withToken: boolean) {
  await page.addInitScript(({ syncKey, personalKey, tokenKey, token, useToken }) => {
    ...
    // Empty personalization means this device has a newer local deletion relative
    // to its mirror. That deletion is sensitive pending data and must never be
    // uploaded to a differently authenticated account.
    ...
    syncKey: SYNC_STORAGE_KEY,
    personalKey: PERSONALIZATION_STORAGE_KEY,
    tokenKey: TOKEN_KEY,
    token: VALID_SHAPE_TOKEN,
    useToken: withToken,
  });
}
...
test.describe("Phase 1 private-sync and storage integrity", () => {
  ...
  test("a differently authenticated account receives zero pending uploads", async ({ page }) => {
    ...
    await page.route("**/unreached-sync/private/sync", async (route) => {
      syncPosts += 1;
      await route.fulfill({ status: 500, contentType: "application/json", body: JSON.stringify({ error: "Upload must not occur" }) });
    });
  ...
  test("a new tab without its session token pauses the existing device binding", async ({ page }) => {
    ...
    await page.route("**/unreached-sync/private/**", async (route) => {
      privateRequests += 1;
      await route.fulfill({ status: 500, contentType: "application/json", body: JSON.stringify({ error: "No authenticated private request expected" }) });
    });
