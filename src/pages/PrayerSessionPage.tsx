...
import { buildLivePrayerProfile, livePrayerContextSummary, livePrayerFlow, livePrayerPlainReason, useLivePrayerExperience } from "../prayer";
...
export function PrayerSessionPage() {
  ...
  if (prayer.loading || (prayer.ready && !planInitialized)) {
    return <section class="prayer-session v3-prayer-session prayer-state" role="status">Preparing private prayer session{prayer.progress ? `… ${prayer.progress.loadedPages}/${prayer.progress.totalPages}` : "…"}</section>;
  }

  if (prayer.error) {
    return (
      <section class="prayer-session v3-prayer-session prayer-session--state">
  ...
  if (!entity || !profile || !context) {
    return (
      <section class="prayer-session v3-prayer-session prayer-session--state">
        <Database size={25} aria-hidden="true" />
        <div class="eyebrow">Guided prayer session</div>
        <h1 class="display-title">A session person is no longer available.</h1>
        <p>The frozen session plan points to a person whose current live record could not be resolved. Return to Prayer to start a fresh session from current source data.</p>
        <a class="prayer-session__primary-link" href={hrefFor("/pray")}>Return to Prayer</a>
      </section>
    );
  }
  ...
  return (
    <article class="prayer-session v3-prayer-session" data-v3-prayer="session" data-prayer-session-plan={planIds.join(",")}>
      <nav class="prayer-session__back v3-prayer-breadcrumb" aria-label="Prayer session navigation">
        <a href={hrefFor("/pray")}><ArrowLeft size={15} aria-hidden="true" /> Prayer</a>
        <span>/</span>
        <a href={hrefFor("/saved")}>My lists</a>
      </nav>
      ...
      <header class="prayer-session__hero v3-prayer-session__hero">
        <div>
          <span class="v3-type-label">Guided prayer session</span>
          <h1 class="v3-type-display-xl">Pray through your rotation.</h1>
          <p class="v3-type-body-lg v3-reading">A {prayerSessionSizeLabel(sessionSize).toLowerCase()} was frozen when this session opened. Recording prayer can update future rotation order, but it will not reshuffle the people already selected here.</p>
        </div>
        <RotateCcw size={32} aria-hidden="true" />
      </header>

      <div class="prayer-session__policy v3-prayer-session__policy">
      ...
      <section class="prayer-session__person v3-prayer-session__person" aria-labelledby="prayer-session-person-heading">
        ...
        <div class="prayer-session__identity">
          <span class="v3-type-label">Current prayer subject</span>
          <h2 id="prayer-session-person-heading" class="v3-type-heading-xl">{profile.peopleName}</h2>
          <p>{context.countryName}{context.languageName ? ` · ${context.languageName}` : ""}</p>
        </div>
        <p class="prayer-session__why">{livePrayerPlainReason(entity)}</p>
        <dl class="v3-prayer-session__facts" aria-label="Current prayer context">
      ...
      <section class="prayer-session__prompts v3-prayer-session__prompts" aria-labelledby="session-prompts-heading">
        <div class="prayer-section-heading"><div><span class="v3-type-label">Compact guide</span><h2 id="session-prompts-heading">Three prayer prompts</h2></div><BookOpen size={21} aria-hidden="true" /></div>
      ...
      <section class="prayer-session__record v3-prayer-session__record" aria-labelledby="session-record-heading">
        <div>
      ...
      <div class="prayer-session__controls v3-prayer-session__controls">
        ...
        {canGoNext ? (
          ...
          <a href={hrefFor("/pray")}>Return to Prayer</a>
        )}
      </div>

      <footer class="prayer-session__footer v3-prayer-session__footer">
