...
const definitions = [
  ...
  ["Mission knowledge graph", "A bounded per-profile relationship explorer connecting source records, countries, languages, mission classifications, resources, taxonomy, related source records and reviewed editorial evidence. Every edge keeps its evidence layer and source basis."],
  ["Source-linked distribution", "Country-level distribution context built from the current PGID plus other PeopleGroups.org records reporting the exact same ROP3 people-name field. This is provider-taxonomy evidence, not a diaspora or universal ethnic-identity claim."],
  ["Guided mission atlas", "A source-grounded Region → Country → People → Prayer learning sequence. Regional prose is limited to current atlas evidence, and pathway selection is deterministic for teaching breadth rather than mission priority."],
  ["Localized shared prayer collections", "A serverless church-sharing format containing only an intentional title, locale, version and public PeopleGroups PEIDs. Private notes, prayer history, account data and sync metadata are never part of the payload."],
    ...
    <article class="about-page">
      ...
      <section class="about-section" aria-labelledby="method-heading">
        ...
        <ol class="about-method-list">
          ...
          <li><strong>Provider semantics stay provider semantics.</strong><span>GSEC remains IMB GSEC. Joshua Project LeastReached and JP Scale remain Joshua Project fields. Unreached does not translate one methodology into the other.</span></li>
          <li><strong>Cross-source identity is reviewed, not guessed.</strong><span>Provider comparison requires an explicit country-and-identity crosswalk. Similar names and coincident numeric IDs are never enough on their own.</span></li>
          ...
          <li><strong>Graph edges retain provenance.</strong><span>The mission knowledge graph keeps provider facts, source taxonomy, derived taxonomy matches and reviewed editorial evidence as separate layers. It does not use hidden similarity scores or treat missing edges as proof of absence.</span></li>
          <li><strong>Distribution is not diaspora.</strong><span>Advanced geographic intelligence may show country contexts linked by an exact ROP3 source-taxonomy match. It does not infer migration routes, city settlements, diaspora identity, origin/destination history or global population shares.</span></li>
          <li><strong>Guides teach; they do not rank.</strong><span>Guided atlas pathways use stable source-backed rules to create a readable Region → Country → People → Prayer sequence. Reviewed editorial coverage changes teaching depth only and never becomes a mission-priority score.</span></li>
          <li><strong>Church sharing exports references, not private memory.</strong><span>Shared prayer links encode only a title, display locale, version and public PEIDs. Current source data is re-resolved when opened; notes, prayer timestamps, prayer memory, account identity and sync metadata remain private.</span></li>
