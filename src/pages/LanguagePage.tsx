...
import { LanguageIntelligencePanel } from "../components/LanguageIntelligencePanel";
...
export function LanguagePage({ iso6393 }: { iso6393: string }) {
  ...
  return (
    <article class="language-page">
      ...
      <LanguageIntelligencePanel record={record} languages={explorer.languages} />
      ...
      <div class="language-profile-grid">
        ...
        <aside>
          ...
          <section class="language-section" aria-labelledby="language-taxonomy-heading"><div class="language-section__heading"><div><span class="eyebrow">Linguistic identity</span><h2 id="language-taxonomy-heading">Source identity</h2></div><Languages size={20} aria-hidden="true" /></div><dl class="language-fact-list"><div><dt>ISO 639-3</dt><dd>{record.iso6393}</dd></div><div><dt>Language name</dt><dd>{record.name}</dd></div><div><dt>Family</dt><dd>{record.family.breakdown.length > 1 ? `Mixed source labels · ${record.family.breakdown.map((item) => item.label).join(", ")}` : record.familyName ?? "Not reported"}</dd></div><div><dt>Source contexts</dt><dd>{record.contextCount}</dd></div><div><dt>Population field coverage</dt><dd>{record.populationKnownContextCount}/{record.contextCount}</dd></div></dl></section>
